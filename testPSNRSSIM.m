...
peaks = [120];  % target peak values for the scaled image
...
%             Hyper.RannSig = sigma_den*255;
%             [Iout,NoiseVar,~] = BPFA_Denoise_Mixed(fz,sIMin0/peak,Hyper);
%             Iout(Iout>1)=1;
%             Iout(Iout<0)=0;
...
%             sIMin0hat_alg =  (Iout/2).^2 - 3/8 - sigma^2;                       % algebraic inverse
            dIMname = sprintf('Denoised_PoiGau_%s_peak%d_alpha%d.png',IMname,peak,alpha);
            sIMin0hat=im2double(imread(fullfile(Denoised_image_dir, dIMname)))*peak;
...
%             PSNR_sIMin0hat_alg(i) = 10*log10(peak^2/mean((sIMin0(:)-sIMin0hat_alg(:)).^2));
            %% output
            imwrite(sIMin0hat/peak, ['Denoised_PoiGau_' IMname '_peak' num2str(peak) '_alpha' num2str(alpha) '.png']);
...
%         disp(['Avg output PSNR (algebraic inverse)    = ' num2str(mean(PSNR_sIMin0hat_alg))])
        ...
        result = sprintf('VBPGBPFA_PoiGau_peak%d_alpha%d_sigma%d_Sample%d.mat',peak,alpha,sigma,Sample);
