function [dI,NoiseVar,PSNR] = BPFA_Denoise_Mixed_Real2(IMin,IMin0, Hyper)
%------------------------------------------------------------------
% Input:
%   IMin: noisy image.
...
%             cls_num = max(label);
        ...
        for   i = 1:length(seg)-1
            ...
            for ite = 1:nlsp
                ...
                if ite == 1
                    ...
                    phi = 1/((Hyper.RannSig(channel)/255)^2);
        ...
        Iout = Iout./(im_wei+realmin);
        %% calculate the PSNR
        PSNR = [PSNR csnr( Iout*255, IMin0*255, 0, 0 )];
        SSIM = [SSIM  cal_ssim( Iout*255, IMin0*255, 0, 0 )];
