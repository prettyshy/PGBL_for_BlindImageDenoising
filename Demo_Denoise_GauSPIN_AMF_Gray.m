    ...
    for SpikyRatio = [0.15 0.3]% 0.5
        for Sample=1:1
            ...
            for i =1:im_num
                ...
                %% add "salt and pepper" noise 0 or random-valued impulse noise 1
                %                 rand('seed',0)
                %                 [IMin,Narr]          =   impulsenoise(IMin*255,SpikyRatio,0);
                PSNR          =    csnr( IMin*255, IMin0*255, 0, 0 );
                SSIM          =    cal_ssim(IMin*255, IMin0*255, 0, 0 );
                fprintf('The initial value of PSNR = %2.2f  SSIM=%2.4f\n', PSNR, SSIM);
                %                 fprintf('%s :\n',im_dir(i).name);
                %                 imwrite(IMin, ['../1_Results/NoisyImage/GauSPIN/Noisy_GauSPIN_' IMname '_' num2str(nSig) '_' num2str(SpikyRatio) '_' num2str(Sample) '.png']);
                %% AMF
                IMin = IMin*255;
                [IMinAMF,ind]=adpmedft(IMin,19);
                ind=(IMinAMF~=IMin)&((IMin==255)|(IMin==0));
                IMinAMF(~ind)=IMin(~ind);
                imwrite(IMinAMF/255, ['../1_Results/NoisyImage/GauSPIN/Noisy_AMF2_GauSPIN_' IMname '_' num2str(nSig) '_' num2str(SpikyRatio) '_' num2str(Sample) '.png']);
                %% denoising
                ...
                imwrite(Iout, ['VBPGBPFA_AMF2_GauSPIN_' IMname '_' num2str(nSig) '_' num2str(SpikyRatio) '_' num2str(Sample) '.png']);
            ...
            result = sprintf('VBPGBPFA_AMF2_GauSPIN_%d_%2.2f.mat',nSig,SpikyRatio);
