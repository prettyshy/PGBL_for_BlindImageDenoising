...
for scale = [0.03]
    ...
    for i = 1:im_num
        ...
        fprintf('%s :\n',im_dir(i).name);
        imwrite(IMin, ['../1_Results/NoisyImage/GauLocVar/Noisy_GauLoVa_' IMname '_' num2str(scale) '.png']);
        ...
        imwrite(Iout, ['./GauLocVar/Denoise_GauLoVa_' IMname '_' num2str(scale) '.png']);
        fprintf('%s : PSNR = %2.4f, SSIM = %2.4f \n',im_dir(i).name,csnr( Iout*255, IMin0*255, 0, 0 ),cal_ssim( Iout*255, IMin0*255, 0, 0 ));
    end
    mPSNR = mean(imPSNR);
    mSSIM = mean(imSSIM);
    fprintf('The average PSNR = %2.4f, SSIM = %2.4f. \n', mPSNR,mSSIM);
    result = sprintf('VBPGBPFA_GauLoVa_scale%2.2.mat',scale);
