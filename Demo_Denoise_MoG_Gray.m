...
for i = 1:im_num
    ...
    fprintf('%s :\n',im_dir(i).name);
    imwrite(IMin, ['../1_Results/NoisyImage/MoG/Noisy_MoG_' IMname '_' num2str(nSig(1)) '_' num2str(nWeight(1)) '_' num2str(nSig(2)) '_' num2str(nWeight(2)) '_' num2str(nSig(3)) '_' num2str(nWeight(3)) '.png']);
    %% noise level estimation
    Hyper.RannSig = NoiseLevel(IMin*255);
    fprintf( 'Noisy Image: Noise Level is %2.2f, PSNR = %2.2f \n\n\n',Hyper.RannSig, csnr( IMin*255, IMin0*255, 0, 0 ) );
    %% denoising
    ...
    imwrite(Iout, ['DenoisedMoG_' IMname '_' num2str(nSig(1)) '_' num2str(nWeight(1)) '_' num2str(nSig(2)) '_' num2str(nWeight(2)) '_' num2str(nSig(3)) '_' num2str(nWeight(3)) '.png']);
    fprintf('%s : PSNR = %2.4f, SSIM = %2.4f \n',im_dir(i).name,csnr( Iout*255, IMin0*255, 0, 0 ),cal_ssim( Iout*255, IMin0*255, 0, 0 ));
end
mPSNR = mean(imPSNR);
mSSIM = mean(imSSIM);
result = sprintf('PGBPFA_MoG_%d_%2.2f_%d_%2.2f_%d_%2.2f.mat',nSig(1),nWeight(1),nSig(2),nWeight(2),nSig(3),nWeight(3));
...
for i = 1:im_num
    ...
    IMin = IMin0 + NoiseMatrix;
    fprintf('%s :\n',im_dir(i).name);
    imwrite(IMin, ['NoisyMoG_' IMname '_' num2str(nSig(1)) '_' num2str(nWeight(1)) '_' num2str(nSig(2)) '_' num2str(nWeight(2)) '_' num2str(nSig(3)) '_' num2str(nWeight(3)) '.png']);
    %% noise level estimation
    Hyper.RannSig = NoiseLevel(IMin*255);
    fprintf( 'Noisy Image: Noise Level is %2.2f, PSNR = %2.2f \n\n\n',Hyper.RannSig, csnr( IMin*255, IMin0*255, 0, 0 ) );
    %% denoising
