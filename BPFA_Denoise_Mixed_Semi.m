function [IMin,NoiseVar,PSNR] = BPFA_Denoise_Mixed_Semi(IMinAMF,IMin,IMin0, Hyper)
%------------------------------------------------------------------
% Input:
%   IMin: noisy image.
...
while loop && Iteration <= MaxIteration
    ...
    %% Clustering via GMM
    [nDCSeedXAMF,nDCnlXAMF,blk_arr,~] = CalNonLocal(IMinAMF,PatchSize, step,nlsp);
    [~,nDCnlX,~,DC] = CalNonLocal(IMin,PatchSize, step,nlsp);
    if Iteration ==1
        label = vbgm(nDCSeedXAMF, cls_num);
        cls_num = max(label);
        fprintf('Iter %d : There are %d components.\n',Iteration, cls_num);
        label = emgm(nDCnlXAMF, cls_num,nlsp);
    else
        label = emgm(nDCnlXAMF, cls_num,nlsp);
    end
